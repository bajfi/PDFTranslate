...
def translate_stream(
    ...
    **kwarg: Any,
    ...
    with tqdm.tqdm(total=total_pages, desc="Translating PDF") as progress_bar:
        for _, chunk_pages in enumerate(page_chunks):
            if cancellation_event and cancellation_event.is_set():
                raise CancelledError("task cancelled")
            ...
            if not skip_subset_fonts:
                try:
                    doc_zh.subset_fonts(fallback=True)  # Bad value error
    ...
    for page_idx in tqdm.tqdm(range(doc_en_final.page_count), desc="Merging document"):
        # Get original and translated pages
        page_en = doc_en_final[page_idx]
        page_zh = doc_zh_final[page_idx]

        # Get page dimensions
        rect_en = page_en.rect
        rect_zh = page_zh.rect

        # Create new page with combined width
        new_width = rect_en.width + rect_zh.width
        new_height = max(rect_en.height, rect_zh.height)
        new_page = doc_dual.new_page(width=new_width, height=new_height)

        # Insert original page on the left
        new_page.show_pdf_page(
            pymupdf.Rect(0, 0, rect_en.width, rect_en.height), doc_en_final, page_idx
        )

        # Insert translated page on the right
        new_page.show_pdf_page(
            pymupdf.Rect(rect_en.width, 0, new_width, rect_zh.height),
            doc_zh_final,
            page_idx,
        )
